...
import com.baidu.hugegraph.store.cmd.UpdatePartitionResponse;
...
public class HgStoreEngine implements Lifecycle<HgStoreEngineOptions>, HgStoreStateListener {
    ...
    public static HgStoreEngine getInstance() { return instance; }
    ...
    public synchronized boolean init(final HgStoreEngineOptions opts) {
        ...
        hgCmdClient.init(new RpcOptions(), (graphName, ptId) -> {
            ...
            if (getPartitionEngine(ptId) != null) {
                return getPartitionEngine(ptId).waitForLeader(options.getWaitLeaderTimeout() * 1000);
            } else {
                // 可能出现跨分区的迁移
                Metapb.Shard shard = pdProvider.getPartitionLeader(graphName, ptId);
                return JRaftUtils.getEndPoint(pdProvider.getStoreByID(shard.getStoreId()).getRaftAddress());
            }
        });
    ...
    private RpcServer createRaftRpcServer(String raftAddr) {
        ...
        RpcServer rpcServer = RaftRpcServerFactory.createRaftRpcServer(endpoint,
                JRaftUtils.createExecutor("RAFT-RPC-", options.getRaftRpcThreadPoolSize()),
                null);
    ...
    public void shutdown() {
        ...
        rpcServer.shutdown();
        // HgStoreEngine.init function check rpcServer whether is null, skipped if the instance exists even shut down.
        rpcServer = null;
        ...
        try {
            if (!options.isFakePD()) {  // FakePD模式不需要同步
                partitionManager.syncPartitionsFromPD(partition -> {
                    log.warn("The local partition information is inconsistent with the PD server. " +
            ...
            partitionManager.getPartitions().forEach((k, g) -> {
                g.forEach((id, p) -> {
                    try {
                        ...
                        log.error("Partition {}-{} restore  exception {}", p.getGraphName(), p.getId(), e);
                    }
                });
            });
        } catch (PDException e) {
    ...
    private PartitionEngine createPartitionEngine(int groupId, ShardGroup shardGroup, Configuration conf){
        ...
        if ((engine = partitionEngines.get(groupId)) == null) {
            synchronized (this) {
                ...
                if (conf != null){
                    ...
                    if (! list.stream().anyMatch(p -> p.getEndpoint().toString().equals(options.getRaftAddress())))  {
                        log.info("raft {}, conf {} does not contains raft address:{}, skipped create partition engine",
        ...
        if (engine == null) {
            ...
            if (shardGroup != null) {
                ...
                shardGroup.getShards().forEach((shard) -> {
                    ...
                    hgCmdClient.createRaftNode(store.getRaftAddress(), Arrays.asList(partition),
                            status -> {
                                log.info("send to {} createRaftNode rpc call result {} partitionId {}",
                                        store.getRaftAddress(), status, partition.getId());
                            });
                });
            }
        } else {
    ...
    public void destroyPartitionGroups(Partition partition) {
        ...
        if (shardGroup != null) {
            shardGroup.getShards().forEach((shard) -> {
                ...
                // 向其他peer发消息，创建raft 分组。此处是异步发送
                hgCmdClient.destroyRaftNode(store.getRaftAddress(), Arrays.asList(new Partition[]{partition}),
                        status -> {
    ...
    public DataMover getDataMover(){ return dataMover; }
    ...
    public HgCmdClient getHgCmdClient(){ return hgCmdClient;}

    public HeartbeatService getHeartbeatService() { return heartbeatService; }

    public boolean isClusterReady() { return heartbeatService.isClusterReady();}
     ...
     * @param operation
     */
    public void addRaftTask(String graphName, Integer partId, RaftOperation operation, RaftClosure closure) {
        ...
        if (engine != null) {
            ...
            if (engine.isLeader()) {
                ...
                Store store = partitionManager.getStoreByRaftEndpoint(engine.getShardGroup(), leader.toString());
                if (store.getId() == 0){
                ...
                closure.run(new Status(HgRaftError.NOT_LEADER.getNumber(),
                        String.format("Partition %s-%d leader changed to %x", graphName, partId, store.getId())));
                log.error("Raft Partition {}-{} not leader, redirectTo leader {}.", graphName, partId, leader);
            } else {
            ...
            closure.run(new Status(HgRaftError.NOT_LOCAL.getNumber(), HgRaftError.NOT_LOCAL.getMsg()));
    ...
    class PartitionChangedListener implements PartitionManager.PartitionChangedListener {
        ...
        public void onChanged(Partition partition) {
            ...
            if (engine != null && engine.isLeader()) {
                try {
                    engine.addRaftTask(RaftOperation.create(RaftOperation.SYNC_PARTITION, partition.getProtoObj()),
                            new RaftClosure() {
                                @Override
                                public void run(Status status) {
                                    log.info("Partition {}-{}-{} sync partition status is {}",
                                            partition.getGraphName(), partition.getId(), partition.getWorkState(),
                                            status);
                                }
                            });
                } catch (IOException e) {
        ...
        public UpdatePartitionResponse rangeOrStateChanged(UpdatePartitionRequest request) {
            ...
            try {
                ...
                log.info("not leader request threadId:{} pId:{} range:{}-{} state:{} response:{}",
                            Thread.currentThread().getId(), request.getPartitionId(), request.getStartKey(),
