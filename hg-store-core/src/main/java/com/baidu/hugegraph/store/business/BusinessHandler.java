...
public interface BusinessHandler extends DBSessionBuilder {
    ...
    HgStoreMetric.Partition getPartitionMetric(String graph, int partId, boolean accurateCount) throws HgStoreException;
    ...
    void loadSnapshot(String snapshotPath, String graph, int partId, long version) throws HgStoreException;
    ...
    void ingestSstFile(String graph, int partId, Map<byte[], List<String>> sstFiles) throws HgStoreException;
    ...
    boolean cleanPartition(String graph, int partId, long startKey, long endKey, CleanType cleanType);
    ...
    interface TxBuilder {
        ...
        TxBuilder delRange(int code, String table, byte[] start, byte[] end) throws HgStoreException;
                ...
                if (number == OpType.OP_TYPE_PUT_VALUE) {
                    ...
                    switch (number) {
                        ...
                        case OpType.OP_TYPE_DEL_RANGE_VALUE:
                            builder.delRange(start.getCode(), table, startKey, b.getEndKey().getKey().toByteArray());
                            continue;
                        case OpType.OP_TYPE_DEL_SINGLE_VALUE:
                            builder.delSingle(start.getCode(), table, startKey);
                            continue;
                        case OpType.OP_TYPE_MERGE_VALUE:
                            builder.merge(start.getCode(), table, startKey, b.getValue().toByteArray());
                            continue;
                        default:
                            throw new IllegalArgumentException("unsupported batch-op-type: " + b.getOpType().name());
                    }
                }
            }
            builder.build().commit();
        } catch (Throwable e) {
            String msg = String.format("graph data %s-%s do batch insert with error:", graph, partId);
            log.error(msg, e);
