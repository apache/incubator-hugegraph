...
public class HgStoreStateMachine extends StateMachineAdapter {
    ...
    public void addTaskHandler(RaftTaskHandler handler){ taskHandlers.add(handler); }
        ...
        while (inter.hasNext()) {
            ...
            try {
                for (RaftTaskHandler taskHandler : taskHandlers) {
                    if (done != null) {
                        // Leader分支，本地调用
                        if (taskHandler.invoke(groupId, done.op.getOp(), done.op.getReq(), done.closure)) {
                            done.run(Status.OK());
                            break;
                        }
                    } else {
                ...
                if (done != null) {
                    LOG.error("StateMachine meet critical error: op = {} {}.", done.op.getOp(), done.op.getReq());
    ...
    public long getLeaderTerm(){ return leaderTerm.get(); }
    ...
    public void onLeaderStart(final long term) {
        ...
        Utils.runInThread(() -> {
            stateListeners.forEach(listener -> { listener.onLeaderStart(term); });
        });
    ...
    public void onLeaderStop(final Status status) {
        Utils.runInThread(() -> {
            stateListeners.forEach(listener -> { listener.onLeaderStop(this.leaderTerm.get());});
        });
