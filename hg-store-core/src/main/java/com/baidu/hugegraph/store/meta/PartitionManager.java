...
public class PartitionManager extends GlobalMetaStore {
    ...
    private DeletedFileManager deletedFileManager;
    private boolean useRaft;
    private final HgStoreEngineOptions options;
    private List<PartitionChangedListener> partitionChangedListeners;
    // 读写锁对象
    private ReadWriteLock readWriteLock = new ReentrantReadWriteLock();

    private PartitionMetaStoreWrapper wrapper = new PartitionMetaStoreWrapper();
    ...
    public DeletedFileManager getDeletedFileManager() { return deletedFileManager; }

    public PdProvider getPdProvider() { return pdProvider; }

    public StoreMetadata getStoreMetadata(){ return storeMetadata;}
    ...
    private Boolean resetPartitionPath(int partitionId){
                ...
                if (this.checkPathContains(dbFiles, partitionId, false)){
                    ...
                    if (!location.getStoreLocation().equals(dataPaths.get(i))){
                        Metapb.PartitionStore newLocation = location.toBuilder()
                                .setStoreLocation(dataPaths.get(i))
                                .build();
        ...
        for (int i = 0 ; i < raftPaths.size(); i ++){
            String raftPath = Paths.get(raftPaths.get(i),
                    HgStoreEngineOptions.Raft_Path_Prefix).toAbsolutePath().toString();
            ...
            if (raftFile.exists()) {
                ...
                if (this.checkPathContains(raftFiles, partitionId, true)){
                    ...
                    // 兼容版本升级
                    if (location == null || !Objects.equals(location.getRaftLocation(), raftPaths.get(i))) {
                        Metapb.PartitionRaft newLocation = Metapb.PartitionRaft.newBuilder()
                                .setPartitionId(partitionId)
                                .setRaftLocation(raftPaths.get(i))
                                .build();
    ...
    private void loadPartitions(){
            ...
            for (File f : dirs){
                if (f.isDirectory()) {
                    try {
                        ...
                        log.error("find illegal dir {} in data path, error:{}", f.getName(), e.getMessage());
                    }
                }
            }
        }
        ...
        for (int partId : partIds) {
            if (!resetPartitionPath(partId)) {
                log.error("partition " + partId + " Directory not exists,options " + this.options.getDataPath());
                continue;
            }
            ...
            for (var metaPart : wrapper.scan(partId, Metapb.Partition.parser(), key)) {
                ...
                if (isLegeal) {
                    ...
                    log.info("load partition : {} -{}", partition.getGraphName(), partition.getId());
                }else {
                    // 无效
    ...
    public void syncPartitionsFromPD(Consumer<Partition> delCallback) throws PDException {
        ...
        try {
            ...
            List<Partition> partListFrPD = pdProvider.getPartitionsByStore(storeMetadata.getStore().getId());
    ...
    public void updatePartitionRangeOrState(UpdatePartitionRequest req) {
        ...
        try {
            ...
            if (req.getStartKey() >= 0 && req.getEndKey() > 0
                    && partition.getStartKey() != req.getStartKey() && partition.getEndKey() != req.getEndKey()){
    ...
    public ShardGroup getShardGroup(int partitionId){
        ...
        Metapb.ShardGroup shardGroup = wrapper.get(partitionId, MetadataKeyHelper.getShardGroupKey(partitionId),
        ...
        if (shardGroup == null) {
            ...
            if (shardGroup != null) {
                ...
                wrapper.put(partitionId, MetadataKeyHelper.getShardGroupKey(partitionId), shardGroup.toByteArray());
            } else {
    ...
    // 获取本地Store信息
    public Store getStore(){ return storeMetadata.getStore(); }
    ...
    public Partition getPartition(String graphName, int partitionId) {
        return this.partitions.getOrDefault(graphName, new HashMap<>()).getOrDefault(partitionId, null);
    }
     ...
     * 拆分partition对象
     */
    public List<Metapb.Partition> updatePartitionToPD(List<Metapb.Partition> partitions) throws PDException {
        // 更新本地分区信息，以及cache信息
