...
import com.baidu.hugegraph.store.meta.Shard;
...
import com.baidu.hugegraph.store.options.PartitionEngineOptions;
...
import com.baidu.hugegraph.store.util.FutureClosure;
...
public class PartitionEngine implements Lifecycle<PartitionEngineOptions>, RaftStateListener {
    ...
    private List<PartitionStateListener> stateListeners;
    private Node raftNode;
    private ShardGroup shardGroup;

    private AtomicBoolean changingPeer;

    private AtomicBoolean snapshotFlag;
    private final Object leaderChangedEvent = new String("leaderChangedEvent");
    ...
    public Integer getGroupId() { return options.getGroupId(); }
    ...
    public synchronized boolean init(PartitionEngineOptions opts) {
            ...
            public LogStorage createLogStorage(final String uri, final RaftOptions raftOptions) {
                if ( options.getRaftOptions().isUseRocksDBSegmentLogStorage())
                    return new RocksDBSegmentLogStorage(uri, raftOptions,
                            ...
                            options.getRaftOptions().getMaxSegmentFileSize(),
                            options.getRaftOptions().getPreAllocateSegmentCount(),
                            options.getRaftOptions().getKeepInMemorySegmentCount(),
        ...
        //nodeOptions.setSnapshotLogIndexMargin(options.getRaftOptions().getSnapshotLogIndexMargin());
        ...
        nodeOptions.setRpcInstallSnapshotTimeout(options.getRaftOptions().getRpcInstallSnapshotTimeout());
        ...
        raftOptions.setMaxReplicatorInflightMsgs(options.getRaftOptions().getMaxReplicatorInflightMsgs());
        ...
        // 构建raft组并启动raft
        this.raftGroupService = new RaftGroupService(raftPrefix + options.getGroupId(), serverId, nodeOptions,
        ...
        log.info("PartitionEngine start successfully: id = {}, peers list = {}", options.getGroupId(), nodeOptions.getInitialConf().getPeers());
        return this.started;
    }
    ...
    public ShardGroup getShardGroup() { return shardGroup; }
    ...
    public Status changePeers(List<String> peers, final Closure done) {
        ...
        log.info("Raft {} changePeers start, old peer is {}, new peer is {}", getGroupId(), oldPeers, peers);
        // 检查需要新增的peer。
        List<String> addPeers = ListUtils.removeAll(peers, oldPeers);
        // 需要删除的learner。可能peer发生改变
        List<String> removedPeers = ListUtils.removeAll(RaftUtils.getLearnerEndpoints(raftNode), peers);
        ...
        if (!addPeers.isEmpty()) {
            ...
            addPeers.forEach(peer -> Utils.runInThread(() -> {
                // 1. 创建新peer的raft对象
                rpcClient.createRaftNode(peer, partitionManager.getPartitionList(getGroupId()), conf, status -> {
                    closure.run(status);
                    if (!status.isOk()) {
                        log.error("Raft {} add node {} error {}", options.getGroupId(), peer, status);
                    }
                });
            }));
            closure.get();
        ...
        if (result.isOk()) {
            ...
            if (removedPeers.contains(this.getRaftNode().getNodeId().getPeerId().getEndpoint().toString())) {
                ...
                log.info("Raft {} leader is removed, needs to transfer leader {}, conf: {}", getGroupId(), peers, conf);
        ...
        if (!RaftUtils.configurationEquals(oldConf, conf)) {
                ...
                if (!removedPeers.isEmpty()) {
                    removedPeers.forEach(peer -> Utils.runInThread(() -> {
                        ...
                        rpcClient.destroyRaftNode(peer, partitionManager.getPartitionList(getGroupId()),
                            ...
                            if (!status.isOk()) {
                                // TODO 失败了怎么办？
                                log.error("Raft {} destroy node {} error {}", options.getGroupId(), peer, status);
                            }
                        });
                    }));
                }
            ...
            log.info("Raft {} changePeers result {}, conf is {}", getRaftNode().getGroupId(), closure.get(), conf);
        }
    ...
    public void checkActivity(){
        Utils.runInThread(() -> {
           if ( !this.raftNode.getNodeState().isActive()){
               log.error("Raft {} is not activity state is ", this.getGroupId(), raftNode.getNodeState());
               restartRaftNode();
           }
        });
    }
    ...
    public void destroy() {
        ...
        try {
            ...
            if (! Objects.equals(this.options.getRaftDataPath(), this.options.getRaftSnapShotPath())) {
    ...
    public Map<Long, PeerId> getAlivePeers() {
        ...
        raftNode.listAlivePeers().forEach(peerId -> {
            Shard shard = partitionManager.getShardByRaftEndpoint(shardGroup, peerId.getEndpoint().toString());
            if (shard != null)
            ...
            while ((System.currentTimeMillis() - start < timeOut) && (leader == null)) {
                ...
                if (leader == null){
                    if ( partitionManager.isLocalPartition(this.options.getGroupId())) {
                        log.error("Raft {} leader not found, try to repair!", this.options.getGroupId());
                        // TODO 判断raft是否本机，如果是，尝试修复Leader，包括检查配置是否正确
                        storeEngine.createPartitionGroups(partitionManager.getPartitionList(getGroupId()).get(0));
                    }
                }else{
    ...
    public String toString() {
        return "PartitionEngine{" + "groupId =" + this.options.getGroupId() + ", isLeader=" + isLeader()
    ...
    public void onConfigurationCommitted(Configuration conf) {
        ...
        try {
            ...
            log.info("Raft {} onConfigurationCommitted, shardGroup {}", getGroupId(), shardGroup.getMetaPbShard());
    ...
    public void doChangeShard(final MetaTask.Task task, Closure done) {
        ...
        if (taskManager.partitionTaskRepeat(task.getPartition().getId(),
                task.getPartition().getGraphName(), task.getType().name())) {
        ...
        if (task.getState().getNumber() < MetaTask.TaskState.Task_Stop_VALUE && isLeader()) {
            Utils.runInThread(() -> {
                try {
                    // 不能在状态机中changePeers
                    List<String> peers = partitionManager.shards2Peers(task.getChangeShard().getShardList());
                    HashSet<String> hashSet = new HashSet<>(peers);
                    // 任务中有相同的peers，说明任务本身有错误，任务忽略
                    if (peers.size() != hashSet.size()) {
                        log.info("Raft {} doChangeShard peer is repeat， peers：{}", getGroupId(), peers);
                    }
                    Status result;
                    ...
                    if (result.getCode() != HgRaftError.TASK_CONTINUE.getNumber()) {
                        log.info("Raft {} doChangeShard is finished, status is {}", getGroupId(), result);
                        // 任务完成，同步任务状态
                        MetaTask.Task newTask;
                        if (result.isOk()) {
                            newTask = task.toBuilder().setState(MetaTask.TaskState.Task_Success).build();
                        } else {
                            log.warn("Raft {} doChangeShard is failure, need to retry, status is {}",
                                    getGroupId(), result);
                            try{
                                // 减少发送次数
                                Thread.sleep(1000);
                            }catch (Exception e){
                                log.error("wait 1s to resend retry task. got error:{}", e.getMessage());
                            }
                            newTask = task.toBuilder().setState(MetaTask.TaskState.Task_Ready).build();
                        }
                        try {
                            ...
                            if (isLeader()) {
                                storeEngine.addRaftTask(newTask.getPartition().getGraphName(),
                                        ...
                                        RaftOperation.create(RaftOperation.SYNC_PARTITION_TASK, newTask),
                                        status -> {
                                            if (!status.isOk()) {
                                                log.error("Raft {} addRaftTask error, status is {}",
                                                        newTask.getPartition().getId(), status);
                                            }
                                        }
                                );
                            }
    ...
    public Status moveData(MetaTask.Task task) {
        ...
        synchronized (this) {
            ...
            taskManager.updateTask(task.toBuilder().setState(MetaTask.TaskState.Task_Doing).build());
        }
        ...
        log.info("moveData {}-{}-{} result:{}", task.getPartition().getGraphName(), task.getPartition().getId(),
                task.getType().name(), status);

        if (status.isOk()) {
            // 向PD汇报任务执行结果
            partitionManager.reportTask(task.toBuilder().setState(MetaTask.TaskState.Task_Success).build());
            // 更新本地任务状态
            taskManager.updateTask(task.toBuilder().setState(MetaTask.TaskState.Task_Success).build());
        } else {
    ...
    private Status handleSplitTask(MetaTask.Task task){
        ...
        log.info("Partition {}-{} moveData {}", task.getPartition().getGraphName(), task.getPartition().getId(), task);
    ...
    private Status handleMoveTask(MetaTask.Task task){
        Status status;
        try {
            log.info("handleMoveTask: start to copy {} data from {} to {}", task.getPartition().getGraphName(),
    ...
    private synchronized void destroyPartitionIfGraphsNull(String graphName, int partitionId,
                                                           long keyStart, long keyEnd, boolean isLeader) {
        Partition partition = partitionManager.getPartition(graphName, partitionId);

        // key range 校验
        if (partition != null && partition.getEndKey() == keyEnd && partition.getStartKey() == keyStart) {
            log.info("remove partition id :{}, graph:{}", partition.getId(), partition.getGraphName());
        ...
        if (isLeader && partition != null) {
            if (partitionManager.getPartitions(partitionId).size() == 0) {
                log.info("destroyPartitionIfGraphsNull, destroy raft group id:{}", partition.getId());
    ...
    private void handleCleanOp(CleanDataRequest request) {
        ...
        var partition = partitionManager.getPartition(request.getGraphName(), request.getPartitionId());

        if (partition != null) {
            storeEngine.getDataMover().doCleanData(request);
            storeEngine.getBusinessHandler().dbCompaction(partition.getGraphName(), partition.getId());
            ...
            log.info("handleCleanOp, partition is null. {}-{}", request.getGraphName(), request.getPartitionId());
        ...
        if (isLeader) {
            MetaTask.Task task = MetaTask.Task.newBuilder()
                    ...
                    .setCleanPartition(CleanDataRequest.toCleanPartitionTask(request))
            ...
            if (newState == ReplicatorState.ONLINE) {
                ...
                if (task != null)
                    doChangeShard(task, status -> {
                        log.info("Raft {} (replicator state changed) changeShard task {}, status is {}",
    ...
    class TaskHandler implements RaftTaskHandler {
        @Override
        public boolean invoke(final int groupId, byte[] request, RaftClosure response) throws HgStoreException {
            try {
        ...
        @Override
        public boolean invoke(final int groupId, byte methodId, Object req, RaftClosure response) throws HgStoreException {
            ...
            switch (methodId) {
                case RaftOperation.SYNC_PARTITION_TASK: {
                    ...
                    switch (task.getType()) {
                        case Change_Shard:
                            log.info("change shard task: id {}, change shard:{} ", task.getId(), task.getChangeShard());
                ...
                case RaftOperation.DB_COMPACTION:
                    ...
                    storeEngine.getBusinessHandler().dbCompaction(dbCompactionRequest.getGraphName(),
                            dbCompactionRequest.getPartitionId(), dbCompactionRequest.getTableName());
                    break;
                default:
                    return false;
            }
