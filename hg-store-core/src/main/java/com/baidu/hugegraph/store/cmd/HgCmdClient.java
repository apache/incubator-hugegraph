...
public class HgCmdClient {
    ...
    public <T> Future<T> createRaftNode(final String address, final List<Partition> partitions, final Closure done) {
    ...
    public <T> Future<T> destroyRaftNode(final String peer, final List<Partition> partitions, final Closure done) {
        ...
        log.info("Send to {} DestroyRaftNode rpc call  partitionId={} ", peer, request.getPartitionId());
    ...
    public HgCmdBase.BaseResponse tryInternalCallSyncWithRpc(HgCmdBase.BaseRequest request) {
        ...
        for (int i = 0; i < MAX_RETRY_TIMES; i++) {
            try {
                Endpoint leader = ptAgent.getPartitionLeader(request.getGraphName(), request.getPartitionId());
                if (leader == null) {
                    log.error("get leader of graph {} - {} is null", request.getGraphName(), request.getPartitionId());
                    Thread.sleep(i * 1000);
                    continue;
                }
                ...
                if (response != null) {
                    if (response.getStatus().isOK()) {
                        ...
                        log.error("HgCmdClient tryInternalCallSyncWithRpc error msg {} leaders is {}",
...
//                break;
    ...
    private <V> V internalCallSyncWithRpc(final Endpoint endpoint, final HgCmdBase.BaseRequest request)
    ...
    private <V> void internalCallAsyncWithRpc(final Endpoint endpoint, final HgCmdBase.BaseRequest request,
