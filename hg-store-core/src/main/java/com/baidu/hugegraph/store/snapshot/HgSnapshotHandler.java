package com.baidu.hugegraph.store.snapshot;
...
public class HgSnapshotHandler {
        ...
        if (partitionEngine != null){
            ...
            for (String file : files) {
                ...
                if (checksum.length() != 0) {
                    LocalFileMetaOutter.LocalFileMeta meta = LocalFileMetaOutter.LocalFileMeta.newBuilder()
    ...
    public void onSnapshotLoad(final SnapshotReader reader, long committedIndex) throws HgStoreException {
        ...
        businessHandler.loadSnapshot(graphSnapshotDir, "", partitionEngine.getGroupId(), committedIndex);
        ...
        for (Metapb.Partition snapPartition : partitionEngine.loadPartitionsFromLocalDb()){
            ...
            // async tasks
            for (var task : taskManager.scanAsyncTasks(partitionEngine.getGroupId(), snapPartition.getGraphName()) ){
                task.handleTask();
            }
        }
    ...
    public static void findFileList(File dir, File rootDir, List<String> files) {
        ...
        if (fs != null) {
            for (File f : fs) {
                if (f.isFile()) {
                    files.add(trimStartPath(dir.getPath(), rootDir.getPath()) + File.separator + f.getName());
                } else {
    ...
    private void markShouldNotLoad(final Snapshot snapshot, boolean saveSnapshot) {
        ...
        try {
            FileUtils.writeStringToFile(new File(shouldNotLoadPath),
                    saveSnapshot ? "saved snapshot" : "loaded snapshot", Charset.defaultCharset());
        } catch (IOException e) {
            log.error("Failed to create snapshot should not load flag file {}. {}", shouldNotLoadPath, e);
        }
    }
