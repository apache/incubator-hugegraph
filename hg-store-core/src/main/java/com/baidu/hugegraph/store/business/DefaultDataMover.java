...
public class DefaultDataMover implements DataMover{
    ...
    @Override
    public Status moveData(Metapb.Partition source, List<Metapb.Partition> targets) throws Exception {
        Status status = Status.OK();
        // 开始移动数据之前，先把分区下线
        UpdatePartitionResponse response = updatePartitionState(source, Metapb.PartitionState.PState_Offline);
        if (response.getStatus().isOK()) {
            ...
            for (var target : targets){
                if (status.isOk()) {
                    if (!(updatePartitionRange(target, (int) target.getStartKey(), (int) target.getEndKey())
                            .getStatus().isOK()
                            && updatePartitionState(target, Metapb.PartitionState.PState_Normal).getStatus().isOK())){
    ...
    public Status moveData(Metapb.Partition source, Metapb.Partition target) throws Exception {
        // 只写入 target
        return  moveData(source, Arrays.asList(target), (partitions, integer) -> target);
    }
    ...
    private Status moveData(Metapb.Partition source, List<Metapb.Partition> targets,
        ...
        log.info("moveData, graph:{}, partition id:{} tables:{}, {}-{}", source.getGraphName(), source.getId(), tables,
        ...
        for (String table : tables) {
            ...
            try (ScanIterator iterator =
                ...
                while (iterator.hasNext() && status.isOk()) {
                    ...
                    if (partition != null) {
                        moveCount.put(partition.getId(), moveCount.getOrDefault(partition.getId(), 0L) + 1);
                        batch.add(partition.getId(), BatchPutRequest.KV.of(table, code, key, entry.value));
                ...
                for (var pair : moveCount.entrySet()) {
                    log.info("{}-{}, table: {}, move to partition id {}, count:{}, total:{}",
                            source.getGraphName(), source.getId(), table, pair.getKey(), pair.getValue(), total);
                }
            }
        }
    ...
    public UpdatePartitionResponse updatePartitionState(Metapb.Partition partition, Metapb.PartitionState state){
    ...
    public UpdatePartitionResponse updatePartitionRange(Metapb.Partition partition, int startKey, int endKey){
        // 分区分裂时，主动需要查找leader进行同步信息
        UpdatePartitionRequest request = new UpdatePartitionRequest();
