...
public class PartitionEngineTest {
    ...
    public void test(){
        ...
        List<String> oldPeers1 = Arrays.asList(new String[]{"1", "2", "3"});
        List<String> oldLearner1 = Arrays.asList(new String[]{"4", "5"});


        List<String> oldPeers = oldPeers1.stream().map(peer -> peer.toString()).collect(Collectors.toList());
        oldPeers.addAll(oldLearner1.stream().map(peer -> peer.toString()).collect(Collectors.toList()));

        List<String> addedNodes = Arrays.stream(peers).filter(peer ->
                        !oldPeers.stream().map(e -> e.toString()).collect(Collectors.toSet()).contains(peer))
        ...
        List<String> removedNodes = oldPeers.stream().filter(peer ->
                        !Arrays.stream(peers).map(e -> e).collect(Collectors.toSet()).contains(peer))
                .collect(Collectors.toList());
        removedNodes = removedNodes.stream().filter(peer ->
                        !Arrays.stream(learners).map(e -> e).collect(Collectors.toSet()).contains(peer))
                .collect(Collectors.toList());
        ...
        Assert.assertEquals(2, removedNodes.size());
        removedNodes.forEach(s->System.out.print(s + " "));System.out.println();
        // 交集 5
    ...
    public void testUpdateShardsList() {
        ...
        curShards.add(Metapb.Shard.newBuilder().setStoreId(1001).setRole(Metapb.ShardRole.Follower).build());
        curShards.add(Metapb.Shard.newBuilder().setStoreId(1002).setRole(Metapb.ShardRole.Leader).build());
        curShards.add(Metapb.Shard.newBuilder().setStoreId(1003).setRole(Metapb.ShardRole.Follower).build());

        List<Metapb.Shard> reqShards = new ArrayList<>();
        reqShards.add(Metapb.Shard.newBuilder().setStoreId(1001).setRole(Metapb.ShardRole.Leader).build());
        reqShards.add(Metapb.Shard.newBuilder().setStoreId(1002).setRole(Metapb.ShardRole.Leader).build());
        reqShards.add(Metapb.Shard.newBuilder().setStoreId(1004).setRole(Metapb.ShardRole.Leader).build());
        ...
        List<Metapb.Shard> shards = curShards.stream().filter(shard ->
                        reqShards.stream().map(Metapb.Shard::getStoreId).collect(Collectors.toSet()).contains(shard.getStoreId()))
                .collect(Collectors.toList());

        // add
        List<Metapb.Shard> addShards = reqShards.stream().filter(shard ->
                        !curShards.stream().map(Metapb.Shard::getStoreId).collect(Collectors.toSet()).contains(shard.getStoreId()))
                .collect(Collectors.toList());
...
//    @Test
    public void testRpcCall() throws ExecutionException, InterruptedException {
        ...
        Metapb.Partition p = Metapb.Partition.newBuilder()
                .setGraphName("OK")
                .setId(1)
                // .addShards(Metapb.Shard.newBuilder().setStoreId(1).build())
                .build();
