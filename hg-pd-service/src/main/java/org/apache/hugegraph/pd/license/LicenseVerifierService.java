...
@Service
@Slf4j
public class LicenseVerifierService implements ServiceGrpc {
                ...
                if (!installed) {
                    ...
                    if (verifyParam != null) {
                        ...
                        try {
                            ...
                            // 获取有效期，并设置过期时间，通知leader，将content保存到...
                            ...
                            if (!isLeader()) {
                                ...
                                while (RaftEngine.getInstance().getLeader() != null) {
                                    ...
                                    if (info[0] == null) {
                                        ...
                                        if (!error.getType().equals(ErrorType.OK)) {
                            ...
                            throw new PDRuntimeException(ErrorType.LICENSE_ERROR_VALUE,
    ...
    public LicenseContent verify(int cores, int nodeCount) {
        try {
            ...
            if (param != null) {
                if (licCpus != -1) {
                    // licCpus为 -1时，表示不限制cpu核数
                    if (cores <= 0 || cores > licCpus) {
                        String msg =
                                String.format("无效的cpu核数: %s,授权数: %s", cores, licCpus);
                        throw new PDRuntimeException(
                                ErrorType.LICENSE_VERIFY_ERROR_VALUE, msg);
                    }
                }
    ...
    private void checkIpAndMac(ExtraParam param) {
        ...
        if (!matched) {
            throw new PDRuntimeException(ErrorType.LICENSE_VERIFY_ERROR_VALUE, String.format(
