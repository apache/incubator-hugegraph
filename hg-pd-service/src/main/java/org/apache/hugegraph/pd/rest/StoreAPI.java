<<<<<<<< HEAD:hugegraph-pd/hg-pd-service/src/main/java/org/apache/hugegraph/pd/rest/StoreAPI.java
/*
...
>>>>>>>> d7e3d51dd (3.6.5 -> 4.x diff):hg-pd-service/src/main/java/org/apache/hugegraph/pd/rest/StoreAPI.java
...
public class StoreAPI extends API {
    ...
    public RestApiResponse getStores() {
        ...
        try {
            ...
            storeStatsList.sort(Comparator.comparing(o -> o.address));
    ...
    public String setStore(@PathVariable long storeId, @RequestBody StoreRestRequest request) {
        try {
            ...
            if (lastStore != null) {
                ...
                return toJSON(new PDException(ErrorType.NOT_FOUND_VALUE, "error"));
            }
        } catch (PDException e) {
            return toJSON(e);
        }
...
========
            return new RestApiResponse(resultStoreStats, ErrorType.OK, ErrorType.OK.name());
        } else {
            return new RestApiResponse(null, ErrorType.STORE_ID_NOT_EXIST, ErrorType.STORE_ID_NOT_EXIST.name());
>>>>>>>> d7e3d51dd (3.6.5 -> 4.x diff):hg-pd-service/src/main/java/org/apache/hugegraph/pd/rest/StoreAPI.java
        }
    }
...
========
            List<Map<String,Long>> result = pdRestService.getMonitorData(storeId);
            return new RestApiResponse(result, ErrorType.OK, ErrorType.OK.name());
>>>>>>>> d7e3d51dd (3.6.5 -> 4.x diff):hg-pd-service/src/main/java/org/apache/hugegraph/pd/rest/StoreAPI.java
        } catch (PDException e) {
            return new RestApiResponse(null, e.getErrorCode(), e.getMessage());
        }
...
>>>>>>>> d7e3d51dd (3.6.5 -> 4.x diff):hg-pd-service/src/main/java/org/apache/hugegraph/pd/rest/StoreAPI.java
        ...
        StoreStatistics(Metapb.Store store) {
            if (store != null) {
                ...
                try {
                    storeGroupId = pdRestService.getStoreGroupId(store.getId());
                } catch (PDException e) {
                    storeGroupId = -1;
                    log.error("get store group id failed,", e);
                }
