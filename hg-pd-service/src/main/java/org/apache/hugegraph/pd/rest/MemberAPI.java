<<<<<<<< HEAD:hugegraph-pd/hg-pd-service/src/main/java/org/apache/hugegraph/pd/rest/MemberAPI.java
/*
...
>>>>>>>> d7e3d51dd (3.6.5 -> 4.x diff):hg-pd-service/src/main/java/org/apache/hugegraph/pd/rest/MemberAPI.java
...
public class MemberAPI extends API {
    ...
    @ResponseBody
    public RestApiResponse getMembers() throws Exception {

        String leaderGrpcAddress = RaftEngine.getInstance().getLeaderGrpcAddress();
        CallStreamObserverWrap<Pdpb.MembersAndClusterState> response = new CallStreamObserverWrap<>();
        ...
        var stateMap = membersAndClusterState.getStateList().stream()
                .collect(Collectors.toMap(Metapb.GroupClusterState::getStoreGroup,
                        state -> String.valueOf(state.getState())));

        HashMap<String, Object> resultMap = new HashMap<>();
        resultMap.put("state", stateMap.get(DEFAULT_STORE_GROUP_ID));
        resultMap.put("states", stateMap);
        ...
        return new RestApiResponse(resultMap, ErrorType.OK, ErrorType.OK.name());
    }
...
========
            ClusterOp.ChangePeerListRequest rpcRequest = ClusterOp.ChangePeerListRequest.newBuilder().setPeerList(
                    body.getPeerList()).build();
            CountDownLatch latch = new CountDownLatch(1);
            final ResponseHeader[] responseHeader = {null};
            StreamObserver<ClusterOp.ChangePeerListResponse> observer = new StreamObserver<>() {
                @Override
                public void onNext(ClusterOp.ChangePeerListResponse value) {
                    responseHeader[0] = value.getHeader();
                }

                @Override
                public void onError(Throwable t) {
                    responseHeader[0] = ResponseHeader.newBuilder().setError(
                            Errors.newBuilder().setType(
                                    ErrorType.UNKNOWN).setMessage(
                                    t.getMessage()).build()).build();
                    latch.countDown();
                }
>>>>>>>> d7e3d51dd (3.6.5 -> 4.x diff):hg-pd-service/src/main/java/org/apache/hugegraph/pd/rest/MemberAPI.java
