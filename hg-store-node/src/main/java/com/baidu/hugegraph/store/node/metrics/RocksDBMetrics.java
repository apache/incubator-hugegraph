...
import static com.baidu.hugegraph.store.node.metrics.RocksDBMetricsConst.*;
...
public class RocksDBMetrics {
    ...
    private static void registerHistogram(String graph, MeterRegistry registry, HistogramType
        ...
        saveGraphMeter(graph, Gauge.builder(baseName + ".max", histogram, HistogramDataWrapper::getMax).tag("graph", graph).register(registry));
        saveGraphMeter(graph, Gauge.builder(baseName + ".mean", histogram, HistogramDataWrapper::getAverage).tag("graph", graph).register(registry));
        saveGraphMeter(graph, Gauge.builder(baseName + ".min", histogram, HistogramDataWrapper::getMin).tag("graph", graph).register(registry));

        baseName = baseName + ".summary";
        saveGraphMeter(graph, Gauge.builder(baseName, histogram, HistogramDataWrapper::getMedian).tags("graph", graph, LABELS, LABEL_50).register(registry));
        saveGraphMeter(graph, Gauge.builder(baseName, histogram, HistogramDataWrapper::getPercentile95).tags("graph", graph, LABELS, LABEL_95).register(registry));
        saveGraphMeter(graph, Gauge.builder(baseName, histogram, HistogramDataWrapper::getPercentile99).tags("graph", graph, LABELS, LABEL_99).register(registry));
        saveGraphMeter(graph, Gauge.builder(baseName + ".sum", histogram, HistogramDataWrapper::getSum).tags("graph", graph).register(registry));
        saveGraphMeter(graph, Gauge.builder(baseName + ".count", histogram, HistogramDataWrapper::getCount).tags("graph", graph).register(registry));

    }

    private static void registerMemoryUse() {
        Gauge.builder(PREFIX + ".table.reader.total", memoryUseWrapper, (e) -> e.getTableReaderTotal())
                .description("The current number of threads in the pool.")
                .register(registry);
        Gauge.builder(PREFIX + ".mem.table.total", memoryUseWrapper, (e) -> e.getMemTableTotal())
                .description("The current number of threads in the pool.")
                .register(registry);
        Gauge.builder(PREFIX + ".mem.table.unFlushed", memoryUseWrapper, (e) -> e.getMemTableUnFlushed())
                .description("The current number of threads in the pool.")
                .register(registry);
        Gauge.builder(PREFIX + ".cache.total", memoryUseWrapper, (e) -> e.getCacheTotal())
                .description("The current number of threads in the pool.")
                .register(registry);
        Gauge.builder(PREFIX + ".block.cache.pinned-usage", memoryUseWrapper, (e) -> e.getProperty("rocksdb.block-cache-pinned-usage"))
                .description("The current number of threads in the pool.")
                .register(registry);
    ...
    private static void registrySessionRefNum(String graph) {
        ...
        saveGraphMeter(graph,
                Gauge.builder(PREFIX + ".session.ref.count", sessionWrapper, (e) -> e.getRefCount() - 1)
    ...
    private static class StatisticsWrapper {
        ...
        private final String graphName;
        long lastTime = 0;
        private Map<TickerType, Long> tickerCounteMap = new ConcurrentHashMap<>();
        private Map<HistogramType, HistogramData> histogramDataMap = new ConcurrentHashMap<>();
        ...
        private void loadData(){
            ...
            try (RocksDBSession session = getRocksDBSession(graphName)) {
                if (session == null) {
                    //   log.error("Failed to fetch the RocksDBSession with graph's name: [ " + graph + " ]");
                    return ;
                }
    ...
    private static class HistogramDataWrapper {
        ...
        private Supplier<HistogramData> supplier;
