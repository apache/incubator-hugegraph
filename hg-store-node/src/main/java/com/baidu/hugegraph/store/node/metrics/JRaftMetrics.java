...
public class JRaftMetrics {
    ...
    private static void registerNodeMetrics() {
        ...
        synchronized (groupSet) {
            map.forEach((group, metrics) -> {
                ...
                metrics.getMetricRegistry().getGauges().forEach((k, v) -> registerGauge(group, k, v));
                metrics.getMetricRegistry().getMeters().forEach((k, v) -> registerMeter(group, k, v));
                metrics.getMetricRegistry().getCounters().forEach((k, v) -> registerCounter(group, k, v));
                metrics.getMetricRegistry().getTimers().forEach((k, v) -> registerTimer(group, k, v));
                metrics.getMetricRegistry().getHistograms().forEach((k, v) -> registerHistogram(group, k, v));
            });
        }
    ...
    private static void registerHistogram(String group, String name, com.codahale.metrics.Histogram histogram) {
        ...
        Gauge.builder(baseName + ".sum", wrapper, (d) -> Arrays.stream(d.getSnapshot().getValues()).sum())
                .tags(tags).register(registry);
    ...
    private static void registerCounter(String group, String name, com.codahale.metrics.Counter counter) {
        List<Tag> tags = new LinkedList<>();
        tags.add(handleDataTag);
        tags.add(Tag.of("group", group));
    ...
    private static void registerGauge(String group, String name, com.codahale.metrics.Gauge<?> gauge) {
        List<Tag> tags = new LinkedList<>();
        tags.add(handleDataTag);
        tags.add(Tag.of("group", group));
