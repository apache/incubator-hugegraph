...
public class PDClient {
    ...
    static class StubProxy {
        ...
        public void set(PDGrpc.PDBlockingStub stub){ this.stub = stub;}
        public PDGrpc.PDBlockingStub get(){return this.stub;}
        public String getHost(){ return hostList.peek();}
        public int getHostCount(){ return hostList.size();}
    }
    /**
    ...
    private synchronized  void newBlockingStub() throws PDException {
        ...
        PDWatch pdWatch = new PDWatchImpl(host);
        if (config.isEnableCache()) {
            ...
            storeWatcher = pdWatch.watchNode(new PDWatch.Listener<NodeEvent>() {
                @Override
                public void onNext(NodeEvent response) {
                    log.info("PDClient receive store event {} {}", response.getEventType(), Long.toHexString(response.getNodeId()));
        ...
        graphWatcher = pdWatch.watchGraph(new PDWatch.Listener<WatchResponse>() {
            @Override
        ...
        shardGroupWatcher = pdWatch.watchShardGroup(new PDWatch.Listener<>() {
            ...
            public void onNext(WatchResponse response) {
                ...
                log.info("PDClient receive shard group event: raft {}-{}", shardResponse.getShardGroupId(),
                ...
                if (config.isEnableCache()) {
                    switch (shardResponse.getType()) {
                        ...
                        case WATCH_CHANGE_TYPE_ALTER:
                            cache.updateShardGroup(response.getShardGroupResponse().getShardGroup());
                            break;
                        default:
                                break;
                    }
    ...
    private synchronized void closeStub(boolean closeWatcher) {
        ...
        partitionWatcher = storeWatcher = null;
     ...
     * @throws PDException
     */
    public KVPair<Metapb.Partition, Metapb.Shard> getPartitionByCode(String graphName, long hashCode)
            throws PDException {
        // 先查cache，cache没有命中，在调用PD
    ...
    public KVPair<Metapb.Partition, Metapb.Shard> getPartitionById(String graphName, int partId) throws PDException {
        KVPair<Metapb.Partition, Metapb.Shard> partShard = cache.getPartitionById(graphName, partId);
        if (partShard == null) {
    ...
    public void updateShardGroup(Metapb.ShardGroup shardGroup) throws PDException {
        Pdpb.UpdateShardGroupRequest request = Pdpb.UpdateShardGroupRequest.newBuilder()
                ...
                .build();
        Pdpb.UpdateShardGroupResponse response = blockingUnaryCall(PDGrpc.getUpdateShardGroupMethod(), request);
        handleResponseError(response.getHeader());
    ...
    private <ReqT, RespT, StubT extends AbstractBlockingStub<StubT>> RespT
    blockingUnaryCall(MethodDescriptor<ReqT, RespT> method, ReqT req, int retry) throws PDException {
        io.grpc.stub.AbstractBlockingStub<StubT> stub = (AbstractBlockingStub<StubT>) getStub();
        try {
            RespT resp = io.grpc.stub.ClientCalls.blockingUnaryCall(stub.getChannel(), method, stub.getCallOptions(), req);
            return resp;
        } catch (Exception e) {
    ...
    private static void handleResponseError(Pdpb.ResponseHeader header) throws
                                                                        PDException {
        if (header.hasError() && header.getError()
                                       .getType() != Pdpb.ErrorType.OK) {
    ...
    public void setGraphSpace(String graphSpaceName,long storageLimit) throws PDException {
        Metapb.GraphSpace graphSpace = Metapb.GraphSpace.newBuilder().setName(graphSpaceName)
                .setStorageLimit(storageLimit).setTimestamp(System.currentTimeMillis()).build();
    ...
    public void setPDConfig(int partitionCount, String peerList, int shardCount, long version) throws PDException {
        Metapb.PDConfig pdConfig = Metapb.PDConfig.newBuilder().setPartitionCount(partitionCount)
                                                  ...
                                                  .setVersion(version).setTimestamp(System.currentTimeMillis())
                                                  .build();
    ...
    public void changePeerList(String peerList) throws PDException {
        ...
        Pdpb.getChangePeerListResponse response = blockingUnaryCall(PDGrpc.getChangePeerListMethod(), request);
        handleResponseError(response.getHeader());
    }
    ...
    public void splitData() throws PDException {
        Pdpb.SplitDataRequest request = Pdpb.SplitDataRequest.newBuilder()
                .setHeader(header)
                .setMode(Pdpb.OperationMode.Auto).build();
    ...
    public void reportTask(MetaTask.Task task)throws PDException {
        Pdpb.ReportTaskRequest request = Pdpb.ReportTaskRequest.newBuilder()
                .setHeader(header)
                .setTask(task).build();
        Pdpb.ReportTaskResponse response = getStub().reportTask(request);
        handleResponseError(response.getHeader());
    }
    ...
    public void deleteShardGroup(int groupId) throws PDException {
        Pdpb.DeleteShardGroupRequest request = Pdpb.DeleteShardGroupRequest
                ...
                .setGroupId(groupId)
                .build();
        Pdpb.DeleteShardGroupResponse response = getStub().deleteShardGroup(request);
        handleResponseError(response.getHeader());
    }
