<<<<<<<< HEAD:hugegraph-pd/hg-pd-common/src/main/java/org/apache/hugegraph/pd/common/PartitionCache.java
/*
...
>>>>>>>> d7e3d51dd (3.6.5 -> 4.x diff):hg-pd-common/src/main/java/org/apache/hugegraph/pd/common/PartitionCache.java
...
========
            if (! partitionCache.containsKey(graphName)) {
                partitionCache.put(graphName, new ConcurrentHashMap<>());
            }

            partitionCache.get(graphName).put(partId, partition);
...
========
            if (! keyToPartIdCache.containsKey(graphName)) {
                keyToPartIdCache.put(graphName, TreeRangeMap.create());
            }
            keyToPartIdCache.get(graphName).put(Range.closedOpen(partition.getStartKey(),
                    partition.getEndKey()), partId);
...
<<<<<<<< HEAD:hugegraph-pd/hg-pd-common/src/main/java/org/apache/hugegraph/pd/common/PartitionCache.java
            ...
            keyToPartIdCache.computeIfAbsent(graphName, k -> TreeRangeMap.create())
                            .put(Range.closedOpen(partition.getStartKey(), partition.getEndKey()),
                                 partId);
========
            partitionCache.computeIfAbsent(graphName, k -> new ConcurrentHashMap<>()).put(partId, partition);
            keyToPartIdCache.computeIfAbsent(graphName, k -> TreeRangeMap.create())
...
>>>>>>>> d7e3d51dd (3.6.5 -> 4.x diff):hg-pd-common/src/main/java/org/apache/hugegraph/pd/common/PartitionCache.java
    ...
    public boolean hasGraph(String graphName) {
        return !getPartitions(graphName).isEmpty() || getGraph(graphName) != null;
    }
