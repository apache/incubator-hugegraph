<<<<<<<< HEAD:hugegraph-pd/hg-pd-core/src/main/java/org/apache/hugegraph/pd/meta/TaskInfoMeta.java
/*
...
>>>>>>>> d7e3d51dd (3.6.5 -> 4.x diff):hg-pd-core/src/main/java/org/apache/hugegraph/pd/meta/TaskInfoMeta.java
...
public class TaskInfoMeta extends MetadataRocksDBStore {
    ...
    public List<MetaTask.Task> scanUserTask(long taskId) throws PDException {
        byte[] prefix = MetadataKeyHelper.getUserTaskPrefix(taskId);
        return scanPrefix(MetaTask.Task.parser(), prefix);
    }

//    public MetaTask.Task getBuildIndexTask(long taskId, int partitionId) throws PDException {
//        byte[] key = MetadataKeyHelper.getUserTaskKey(taskId, partitionId);
//        return getOne(MetaTask.Task.parser(), key);
//    }

    public void updateUserTask(MetaTask.Task task) throws PDException {
        byte[] key = MetadataKeyHelper.getUserTaskKey(task.getId(), task.getPartition().getId());
        put(key, task.toByteArray());
    }
