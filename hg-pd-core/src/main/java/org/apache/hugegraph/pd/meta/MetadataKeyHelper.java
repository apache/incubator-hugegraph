<<<<<<<< HEAD:hugegraph-pd/hg-pd-core/src/main/java/org/apache/hugegraph/pd/meta/MetadataKeyHelper.java
/*
...
>>>>>>>> d7e3d51dd (3.6.5 -> 4.x diff):hg-pd-core/src/main/java/org/apache/hugegraph/pd/meta/MetadataKeyHelper.java
...
public class MetadataKeyHelper {
    ...
    private static final String TASK_USER = "TASK_USER";
    private static final String LOG_RECORD = "LOG_RECORD";

    private static final String QUEUE = "QUEUE";
    private static final String OBSERVER_NOTICE = "OB_N";
    private static final String NOTICE_CONTENT = "NOTICE_C";
    private static final String STORE_GROUP = "STORE_GROUP";

    private static final String STORE_GROUP_RELATION = "STORE_GROUP_RELATION";
...
========
    public static byte[] getNoticeContentKey(long noticeId) {
        // NOTICE_C/{noticeId}
        StringBuilder builder = StringBuilderHelper.get()
                .append(NOTICE_CONTENT).append(DELIMITER).append(noticeId);

        return builder.toString().getBytes(Charset.defaultCharset());
    }

    public static byte[] getObserverNoticePrefix() {
        // OB_N
        String key = StringBuilderHelper.get()
                .append(OBSERVER_NOTICE).append(DELIMITER)
                .toString();

        return key.getBytes(Charset.defaultCharset());
    }

    public static byte[] getObserverNoticeKey(long observerId, long noticeId) {
        // OB_N/{observerId}/{noticeId}
        StringBuilder builder = StringBuilderHelper.get()
                .append(OBSERVER_NOTICE).append(DELIMITER).append(observerId)
                .append(DELIMITER).append(noticeId);
        return builder.toString().getBytes(Charset.defaultCharset());
    }
...
<<<<<<<< HEAD:hugegraph-pd/hg-pd-core/src/main/java/org/apache/hugegraph/pd/meta/MetadataKeyHelper.java
========
    public static byte[] getUserTaskKey(long taskId, int partitionId) {
        // TASK_BI/ task id / partition id
        StringBuilder builder = StringBuilderHelper.get()
                .append(TASK_USER).append(DELIMITER)
                .append(taskId).append(DELIMITER)
                .append(partitionId);
        return builder.toString().getBytes(Charset.defaultCharset());
    }

    public static byte[] getUserTaskPrefix(long taskId) {
        // TASK_MOVE/{GraphName}/to PartitionID/{source partitionID}
        StringBuilder builder = StringBuilderHelper.get()
                .append(TASK_USER).append(DELIMITER)
                .append(taskId);
...
<<<<<<<< HEAD:hugegraph-pd/hg-pd-core/src/main/java/org/apache/hugegraph/pd/meta/MetadataKeyHelper.java
========
    public static byte[] getStoreGroupRelationKey(Long storeId) {
        StringBuilder builder = StringBuilderHelper.get();
        builder.append(STORE_GROUP_RELATION).append(DELIMITER).append(storeId);
        return builder.toString().getBytes(Charset.defaultCharset());
    }

    public static byte[] getStoreGroupRelationPrefix() {
        StringBuilder builder = StringBuilderHelper.get();
        builder.append(STORE_GROUP_RELATION).append(DELIMITER);
        return builder.toString().getBytes(Charset.defaultCharset());
    }

    public static byte[] getStoreGroupKey(int groupId) {
        StringBuilder builder = StringBuilderHelper.get();
        builder.append(STORE_GROUP).append(DELIMITER).append(groupId);
        return builder.toString().getBytes(Charset.defaultCharset());
    }

    public static byte[] getStoreGroupPrefix() {
        StringBuilder builder = StringBuilderHelper.get();
        builder.append(STORE_GROUP).append(DELIMITER);
        return builder.toString().getBytes(Charset.defaultCharset());
    }
