...
public class StoreMonitorDataService {
    ...
    public void saveMonitorData(Metapb.StoreStats storeStats) throws PDException {
        ...
        if (current - lastStoreStateTimestamp.getOrDefault(storeId, 0L) >= interval){
            ...
            log.debug("store id: {}, system info:{}", storeId, debugMonitorInfo(storeStats.getSystemMetricsList()));
    ...
    private void saveMonitorDataToDb(Metapb.StoreStats storeStats, long ts) throws PDException {
        ...
        log.debug("store id: {}, save monitor data info, ts:{}, my key:{}", storeStats.getStoreId(), ts, key);
     ...
     */
    public Map<String,String> getStoreMonitorData(long storeId, long start, long end) throws PDException {
        log.debug("get monitor data, store id:{}, start{}, end:{}",
                storeId,
                getMonitorDataKey(storeId, start),
                getMonitorDataKey(storeId, end));
        return kvService.scanRange(getMonitorDataKey(storeId, start), getMonitorDataKey(storeId, end));
    }
    ...
    public List<Map<String,Long>> getStoreMonitorData(long storeId) throws PDException {
        ...
        try {
            for(Map.Entry<String, String> entry : getStoreMonitorData(storeId, start, current).entrySet()){
                String[] arr = entry.getKey().split(String.valueOf(MetadataKeyHelper.getDelimiter()));
        ...
        if (result.size() > 0) {
            ...
            for (Map<String, Long> row : result){
                for ( String key : columns){
                    ...
                    if ("ts".equals(key)){
                        // format ts
                        sb.append(dtf.format(LocalDateTime.ofInstant(Instant.ofEpochSecond(row.get(key)),
