...
public class StandardHugeGraph implements HugeGraph {
    ...
    public void initBackend() {
        ...
        try {
            ...
            this.storeProvider.initSystemInfo(this.params);
        } finally {
    ...
    private SysTransaction openSystemTransaction() throws HugeException {
        this.checkGraphNotClosed();
        try {
            return new SysTransaction(this.params, this.loadSystemStore());
        } catch (BackendException e) {
    ...
    private AbstractSerializer serializer() {
        String name = configuration().get(CoreOptions.SERIALIZER);
        LOG.debug("Loading serializer '{}' for graph '{}'", name, name());
    ...
    private Analyzer analyzer() {
        String name = configuration().get(CoreOptions.TEXT_ANALYZER);
        String mode = configuration().get(CoreOptions.TEXT_ANALYZER_MODE);
