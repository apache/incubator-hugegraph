...
public class StreamFinalAggregationIterator<E> implements HgKvIterator<E> {
    ...
    private List<AggregationFunctionParam> aggregationParams;
    ...
    private List<AggregationFunction> getAggregationList() {
        return this.aggregationParams.stream()
                .map((Function<AggregationFunctionParam, AggregationFunction>) param -> {
                    var filedType = param.getFiledType().getGenericType();
                    switch (param.getFunctionType()) {
                        case SUM:
                            return new AggregationFunctions.SumFunction(param.getField(), getSupplier(filedType));
                        case MIN:
                            return new AggregationFunctions.MinFunction(param.getField(), getSupplier(filedType));
                        case MAX:
                            return new AggregationFunctions.MaxFunction(param.getField(), getSupplier(filedType));
                        case AVG:
                            return new AggregationFunctions.AvgFunction(getSupplier(filedType));
                        case COUNT:
                            return new AggregationFunctions.CountFunction();
                        default:
                            throw new RuntimeException("unsupported function type: " + param.getFunctionType());
                    }
                })
