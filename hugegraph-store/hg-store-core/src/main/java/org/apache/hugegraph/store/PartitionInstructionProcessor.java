...
/**
 * PD Sent Tostore's partition instruction processor
 */
public class PartitionInstructionProcessor implements PartitionInstructionListener {
    ...
    public void onChangeShard(long taskId, Partition partition, ChangeShard changeShard,
        ...
        if (engine != null) {
            // Clean up all the tasks, there is a failure situation
    ...
    /**
     * LEDER Receivespd partition division task
     * Add Toraft task queue, RAFT is distributed by task.
     */
    @Override
    public void onSplitPartition(long taskId, Partition partition, SplitPartition splitPartition,
        ...
        if (engine != null && engine.isLeader()) {
            // Answer first to avoid repeated pd delivery caused by overtime
            consumer.accept(0);
    ...
    /**
     * Rocksdb Compaction task sent by Leader Receivespd
     * Add Toraft task queue, RAFT is distributed by task.
     */
    @Override
    ...
    public void onPartitionKeyRangeChanged(long taskId, Partition partition,
        ...
        if (engine != null && engine.isLeader()) {
            ...
            if (localPartition == null) {
                // If the partition data is empty, the local area will not store
